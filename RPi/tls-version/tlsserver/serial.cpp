...
const int portNamesLen = 4;

static int _fd;
static struct termios _serOptions;

void startSerial(int baudRate, int byteSize, char parity, int stopBits, int maxAttempts)
{
	_fd = -1;
	...
	while(_fd < 0 && attempt < maxAttempts)
	{
	  printf("ATTEMPTING TO CONNECT TO SERIAL. ATTEMPT # %d of %d.\n", ++attempt, maxAttempts);
        ...
        for(int i = 0; _fd < 0 && i<portNamesLen; i++){
            _fd=open(portNames[i], O_RDWR | O_NOCTTY | O_NDELAY);
        }
