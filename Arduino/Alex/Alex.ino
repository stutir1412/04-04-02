...
// Variables to keep track of whether we have moved a new distance
...
void setupPowerSaving(){ 
 ...
 PRR |= PRR_TWI_MASK;             // Modify PRR to shut down TWI
 PRR |= PRR_SPI_MASK;            // Modify PRR to shut down SPI
 ADCSRA &= ~ADCSRA_ADC_MASK;    // Modify ADCSRA to disable ADC,
 PRR |= PRR_ADC_MASK;          // then modify PRR to shut down ADC
 SMCR &= SMCR_IDLE_MODE_MASK; // Set the SMCR to choose the IDLE sleep mode
 // Do not set the Sleep Enable (SE) bit yet
 DDRB |= 0b00010000;        // Set Port B Pin 5 as output pin, 
 PORTB &=0b11101111;       //then write a logic LOW 
...
{
 PRR |= (PRR_TIMER1_MASK | PRR_TIMER0_MASK | PRR_TIMER2_MASK);// Modify PRR to shut down TIMER 0, 1, and 2

 SMCR |= SMCR_SLEEP_ENABLE_MASK; // Modify SE bit in SMCR to enable (i.e., allow) sleep

 // The following function puts ATmega328P’s MCU into sleep;
 // it wakes up from sleep when USART serial data arrives
 sleep_cpu();
 SMCR &= ~SMCR_SLEEP_ENABLE_MASK;// Modify SE bit in SMCR to disable (i.e., disallow) sleep
 PRR &= ~(PRR_TIMER1_MASK | PRR_TIMER0_MASK | PRR_TIMER2_MASK);// Modify PRR to power up TIMER 0, 1, and 2
}
...
void forward(float dist, float speed)
{
  ...
  // This will be replaced l ater with bare-metal code.
...
unsigned long computeDeltaTicks(float ang)
{
  ...
  unsigned long ticks = (unsigned long) ((ang * AlexCirc * COUNTS_PER_REV) / (360.0 * WHEEL_CIRC * 3));
